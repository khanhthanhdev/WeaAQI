...
#              the InkyPI service using 'uv' for high-performance setup.
#
...
# ==========================================
# NEW: UV Installation Function
# ==========================================
install_uv_tool() {
    if ! command -v uv &> /dev/null; then
        echo "uv not found. Installing uv..."
        # Install to root's local bin since we are running as sudo
        ...
        if command -v uv &> /dev/null; then
             ...
             echo_error "ERROR: Failed to install uv."
             exit 1
        fi
    else
...
# ==========================================
# UPDATED: Create VENV using UV
# ==========================================
create_venv(){
  echo "Creating python virtual environment using uv."

  # CRITICAL FOR PI ZERO 2 W: Limit concurrency to save RAM
  export UV_CONCURRENCY=4
  
  # Use system python to create venv (faster than downloading managed python)
  # and avoids compatibility issues with system libraries (like libgpiod/RPi.GPIO)
  uv venv "$VENV_PATH" --seed > /dev/null
  
  # Install pip requirements using uv pip interface
  # We explicitly target the venv with -p
  uv pip install -p "$VENV_PATH" -r $PIP_REQUIREMENTS_FILE > /dev/null &
  show_loader "\tInstalling python dependencies (via uv). "
  ...
  if [[ -n "$WS_TYPE" ]]; then
    ...
    uv pip install -p "$VENV_PATH" -r $WS_REQUIREMENTS_FILE > ws_pip_install.log &
    show_loader "\tInstalling additional Waveshare python dependencies (via uv). "
  fi
}
...
# Install UV first so we can use it later
install_uv_tool
...
# Create Venv is now powered by UV
create_venv
